...
class Distance: 
    ...
    def get_dist_dic(self, proximity_sensors, proximity_sensors_handles):

        output_distances = defaultdict(int)
    
        for sensor_name, sensor_handle in zip(proximity_sensors,proximity_sensors_handles):
                err_code,detectionState,detectedPoint,detectedObjectHandle,detectedSurfaceNormalVector=vrep.simxReadProximitySensor(self.clientID,sensor_handle,vrep.simx_opmode_streaming)
                if(err_code == 0):
                    # print("Proximity_sensor_"+sensor_name, np.linalg.norm(detectedPoint))
                    output_distances[sensor_name] = np.linalg.norm(detectedPoint)
                else:
                    output_distances[sensor_name] = float('inf')
